...
def evaluation(img_path, ckpt_path):
  ...
  if ckpt_path:
    ...
    # path = tf.train.latest_checkpoint(ckpt_path)
    vgg_model = main.create_model()
    vgg_model.load_weights(ckpt_path)
  # sess.run(logits)と同じ
  # softmax = logits.eval()
  # 判定結果
  result = vgg_model.predict(image)
  # 判定結果を%にして四捨五入
  rates = [np.round(n * 100.0, 1) for n in result][0]
  humans = []
...
if __name__ == '__main__':
  evaluation('testimage.jpg', './training_3/cp-0005.ckpt')
